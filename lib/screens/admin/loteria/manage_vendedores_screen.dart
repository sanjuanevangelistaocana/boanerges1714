...
class ManageVendedoresLoteriaScreen extends StatelessWidget {
              ...
              child: ConstrainedBox(
                ...
                child: StreamBuilder<List<VendedorLoteria>>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    final cofrades = vendedores.where((v) => v.isCofrade).length;
                        ...
                        Card(
                          ...
                          child: Padding(
                            ...
                            child: Row(
                              ...
                              children: [
                                _MiniStat(value: '${vendedores.length}', label: 'Total', color: AppTheme.primaryColor),
                                _MiniStat(value: '$cofrades', label: 'Cofrades', color: AppTheme.accentColor),
                                _MiniStat(value: '$externos', label: 'Externos', color: Colors.orange.shade700),
                              ],
                            ),
                          ),
                        ),
                          ...
                          child: ListTile(
                            leading: Container(
                              ...
                              decoration: BoxDecoration(
                                color: (v.isCofrade ? AppTheme.accentColor : Colors.orange.shade700).withAlpha(20),
                                borderRadius: BorderRadius.circular(8),
                              ),
                              child: Icon(
                                v.isCofrade ? Icons.person : Icons.store,
                                color: v.isCofrade ? AppTheme.accentColor : Colors.orange.shade700,
                              ),
                            ),
                            ...
                            trailing: PopupMenuButton<String>(
                              onSelected: (action) async {
                                if (action == 'edit') {
                                  _showVendedorDialog(context, fs, vendedor: v);
                                } else if (action == 'delete') {
                                  final confirm = await showDialog<bool>(
                                    context: context,
                                    builder: (ctx) => AlertDialog(
                                      title: const Text('Eliminar vendedor'),
                                      content: const Text('¿Seguro? Se eliminarán también sus asignaciones.'),
                                      actions: [
                                  ...
                                  if (confirm == true) await fs.deleteVendedorLoteria(v.id);
                                }
                              },
                              itemBuilder: (_) => [
  ...
  void _showVendedorDialog(BuildContext context, FirestoreService fs, {VendedorLoteria? vendedor}) {
              ...
              children: [
                DropdownButtonFormField<String>(
                  ...
                  items: const [
                    ...
                    DropdownMenuItem(value: 'externo', child: Text('Externo (establecimiento)')),
                  ],
                  onChanged: (v) => setState(() => tipo = v ?? 'cofrade'),
                ),
                ...
                if (tipo == 'cofrade')
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(labelText: 'Nombre', hintText: 'Ej: Bar Pepe'),
                  ),
                const SizedBox(height: 12),
                TextField(

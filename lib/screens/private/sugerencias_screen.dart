                        ...
                        builder: (context, snapshot) {
                          ...
                          return Column(
                            ...
                            children: [
                              const SizedBox(height: 16),
                              ...conversations.map(
                                  (conversation) => _PrivateConversationCard(
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  children: [
                                    ...
                                    Text(
                                        'Error al cargar env\u00edos: ${snapshot.error}',
                                        textAlign: TextAlign.center,
                                        style: const TextStyle(
                                            color: Colors.red, fontSize: 13)),
                                    const SizedBox(height: 4),
                                    const Text(
                                        'Es posible que se est\u00e9 creando un \u00edndice en Firestore. Int\u00e9ntalo de nuevo en unos minutos.',
                                        textAlign: TextAlign.center,
                                        style: TextStyle(
                                            fontSize: 12,
                          ...
                          if (items.isEmpty) {
                            return Card(
                              ...
                              child: const Padding(
                                ...
                                child: Text(
                                    'No has enviado sugerencias ni peticiones.'),
                              ),
                            );
                          }
                          return Column(
                            children: items
                                .map((s) => _SugerenciaCard(
                                      sugerencia: s,
...
class _PrivateConversationCard extends StatefulWidget {
  final Map<String, dynamic> conversation;
  final String cofradeId;

  const _PrivateConversationCard({
    required this.conversation,
    required this.cofradeId,
  });

  @override
  State<_PrivateConversationCard> createState() =>
      _PrivateConversationCardState();
}

class _PrivateConversationCardState extends State<_PrivateConversationCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final pendingMe = status == 'pending_cofrade';
    final type = '${conversation['type'] ?? 'admin_message'}';
    ...
    return Card(
      ...
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(12),
        side: BorderSide(
          color: pendingMe ? Colors.orange.shade200 : Colors.grey.shade200,
        ),
      ),
      child: Column(
        children: [
          InkWell(
            ...
            child: Padding(
              ...
              child: Row(
                children: [
                  Icon(
                    pendingMe ? Icons.mark_email_unread : Icons.forum_outlined,
                    color: pendingMe
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('${conversation['subject'] ?? 'Comunicación'}',
                            style:
    ...
    return Padding(
      ...
      child: Column(
        children: [
          StreamBuilder<List<Map<String, dynamic>>>(
            stream:
                fs.watchConversationMessages(widget.cofradeId, conversationId),
            builder: (context, snapshot) {
              final messages = snapshot.data ?? [];
              if (messages.isEmpty) {
                return const Text('Todavía no hay mensajes.');
              }
              return Column(
                children: messages.map((message) {
                  final isAdmin =
                      '${message['createdByRole'] ?? ''}'.contains('admin');
                  ...
                  return Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          children: [
                            Text(isAdmin ? 'Administración' : 'Tú',
                                style: const TextStyle(
          ...
          if (isClosed)
            const Align(
              ...
              child: Chip(label: Text('Ticket cerrado')),
            ),
          Row(
            children: [
              Expanded(
                    ...
                    : () async {
                        ...
                        try {
                          await fs.replyConversation(
                            ...
                            createdBy: auth.cofrade?.id ?? widget.cofradeId,
  ...
  String _statusLabel(String status) {
    return switch (status) {
      ...
      'pending_cofrade' => 'Pendiente de contestar',
      'closed' => 'Cerrada',
      _ => 'Abierta',
    };
  }
...
class _SugerenciaCard extends StatefulWidget {
  ...
  const _SugerenciaCard(
      {required this.sugerencia, required this.cofradeNombre});
            ...
            builder: (context, snapshot) {
              ...
              if (mensajes.isEmpty) {
                return Padding(
                  ...
                  child: Text(
                    s.isCerrada
                        ? 'No hubo mensajes en esta conversaci\u00f3n.'
                        : 'A\u00fan no hay mensajes. Escribe el primero o espera la respuesta de la Junta.',
  ...
  Future<void> _cerrarTicket() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
            '\u00bfEst\u00e1s seguro de que quieres cerrar este ticket? No podr\u00e1s enviar m\u00e1s mensajes.'),
        actions: [
          TextButton(
    ...
    if (confirm == true) {
      await context
          ...
          .cerrarSugerencia(widget.sugerencia.id, 'cofrade');
    }
  }
}
